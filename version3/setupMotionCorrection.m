...
addParameter(p,'zPeakMargin',         0.02, @(x) isnumeric(x) && x >= 0);
addParameter(p,'xyFirst',             true, @islogical);
...
MCORR_STATE = struct( ...
    ...
    'zPeakMargin',          o.zPeakMargin, ...
    'xyFirst',              o.xyFirst, ...
    'lastDz_um',            0, ...     % last commanded Z move (µm) — anti-oscillation gate
    'dryRun',               o.dryRun, ...
    'diag',                 o.diag, ...
    'tLastMove',           -inf, ...   % timestamp of last moveSample; Phase 2 is suppressed for postMoveQuiet_s after each move
    'cachedMotorPos',       [], ...    % populated at acqModeStart, then incremented by each commanded move
    'verbose',              o.verbose, ...
    'cumCorr',              [0 0 0]);
